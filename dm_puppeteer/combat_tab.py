...
class CombatTab(QWidget):
    ...
    def __init__(self, state: AppState, bestiary_manager: BestiaryManager,
        ...
        self._reinforcement_pool: list[dict] = []
    ...
    def _build_ui(self):
        ...
        self.remove_btn.setToolTip("Remove from initiative (DM correction)")
        self.remove_btn.setStyleSheet(
            "QPushButton { color: #aa8844; }"
            "QPushButton:hover { background: #3a3a2a; }")
        self.remove_btn.setEnabled(False)
        self.remove_btn.clicked.connect(self._quick_remove)
        ...
        if char_matches:
            ...
            for char in char_matches:
                ...
                if self._reinforcements_mode:
                    btn.clicked.connect(
                        lambda _, c=char:
                            self._add_character_to_reinforcement_pool(c))
                else:
                    btn.clicked.connect(
        ...
        if bestiary_matches:
            ...
            for entry in bestiary_matches:
                ...
                if self._reinforcements_mode:
                    btn.clicked.connect(
                        lambda _, e=entry:
                            self._add_monster_to_reinforcement_pool(e))
                else:
                    btn.clicked.connect(
    ...
    def _add_monster_from_search(self):
        ...
        if self._reinforcements_mode:
            self._add_monster_to_reinforcement_pool(entry)
        else:
    ...
    def _roster_change_count(self, monster_index, delta):
        """Change count for a monster by its monster-list index."""
        target = (self._reinforcement_pool if self._reinforcements_mode
                  else self._combat_roster)
        monster_items = [r for r in target if r["type"] == "monster"]
        if 0 <= monster_index < len(monster_items):
            monster_items[monster_index]["count"] = max(
                1, monster_items[monster_index]["count"] + delta)
            if self._reinforcements_mode:
                self._rebuild_reinforcement_pool_ui()
            else:
                self._rebuild_roster_ui()
    ...
    def _roster_remove_monster(self, monster_index):
        ...
        monster_items = [r for r in target if r["type"] == "monster"]
        if 0 <= monster_index < len(monster_items):
            target.remove(monster_items[monster_index])
            if self._reinforcements_mode:
                self._rebuild_reinforcement_pool_ui()
                self._update_reinforcement_button_state()
            else:
                self._rebuild_roster_ui()
    ...
    def _roster_remove_by_character(self, character_id):
        ...
        if self._reinforcements_mode:
            self._reinforcement_pool = [
                r for r in self._reinforcement_pool
    ...
    def _show_reinforcements_mode(self):
        ...
        # Relabel
        self.roster_label.setText("Reinforcements Pool:")
        self.roster_add_btn.setText("+ Add")
        self.monster_search.setPlaceholderText(
            "Search to add reinforcements...")
        self.reinforcements_hint.setVisible(True)
        self.reinforcements_hint.setText(
            "Stage reinforcements above, then click\n"
            "'Add Reinforcements!' to commit them all.")

        # Transform the big button
        try:
            self.roll_initiative_btn.clicked.disconnect()
        except TypeError:
            pass
        self.roll_initiative_btn.clicked.connect(
            self._commit_reinforcements)
        self.roll_initiative_btn.setText("Add Reinforcements!")
        self.roll_initiative_btn.setEnabled(False)
        self.roll_initiative_btn.setStyleSheet(
            "QPushButton { background: #2a4a5a; border: 2px solid #4488aa; "
            "border-radius: 6px; padding: 10px 20px; color: #fff; "
            "font-size: 14px; font-weight: bold; }"
            "QPushButton:hover { background: #3a5a6a; }"
    ...
    def _show_roster_mode(self):
        ...
        # Restore the big button
        try:
            self.roll_initiative_btn.clicked.disconnect()
        except TypeError:
            pass
        self.roll_initiative_btn.clicked.connect(self._start_combat)
        self.roll_initiative_btn.setText("Roll Initiative!")
    ...
    def _add_character_to_reinforcement_pool(self, char):
        """Add a character to the reinforcement staging pool."""
        for item in self._reinforcement_pool:
            if item.get("character_id") == char.id:
                self.status_message.emit(
                    f"{char.name} is already in the pool!", 3000)
                return

        self._reinforcement_pool.append({
            "type": "character",
            "name": char.name,
            "character_id": char.id,
            "pc_slot_id": "",
    ...
    def _add_monster_to_reinforcement_pool(self, entry):
        """Add a bestiary monster to the reinforcement staging pool."""
        self.bestiary_manager.record_usage(entry.name)

        # Increment count if already in pool
        for item in self._reinforcement_pool:
            if item["type"] == "monster" and item["name"] == entry.name:
                item["count"] += 1
                self._rebuild_reinforcement_pool_ui()
                self.monster_search.clear()
                self.search_results.setVisible(False)
    ...
    def _rebuild_reinforcement_pool_ui(self):
        """Rebuild the reinforcement pool display in the roster scroll area."""
        while self.roster_layout.count():
            child = self.roster_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()

        if not self._reinforcement_pool:
            placeholder = QLabel(
                "Search above to stage reinforcements.\n"
                "Click 'Add Reinforcements!' to commit them all.")
            placeholder.setStyleSheet("color: #666; padding: 12px;")
            placeholder.setWordWrap(True)
            self.roster_layout.addWidget(placeholder)
            return

        char_items = [r for r in self._reinforcement_pool
                      if r["type"] in ("pc", "character")]
        monster_items = [r for r in self._reinforcement_pool
                         if r["type"] == "monster"]

        for item in char_items:
            self._build_pc_roster_row(item)

        if char_items and monster_items:
            sep = QFrame()
            sep.setFrameShape(QFrame.Shape.HLine)
            sep.setStyleSheet("color: #444;")
            self.roster_layout.addWidget(sep)
    ...
    def _update_reinforcement_button_state(self):
        """Enable/disable 'Add Reinforcements!' based on pool contents."""
        self.roll_initiative_btn.setEnabled(
            bool(self._reinforcement_pool))

    def _commit_reinforcements(self):
        """Commit the reinforcement pool to active combat.

        Prompts for a single initiative value for the whole batch,
        converts all pool entries to combatants, inserts them into
        the initiative order, then clears the pool.
        """
        if not self._reinforcement_pool:
            return

        # Count total reinforcements for the prompt
        total = sum(
            item.get("count", 1) if item["type"] == "monster" else 1
            for item in self._reinforcement_pool
        )
        ...
        names = []
        for item in self._reinforcement_pool:
            if item["type"] == "monster" and item["count"] > 1:
                names.append(f"{item['name']} x{item['count']}")
            else:
                names.append(item["name"])
        summary = ", ".join(names)

        # Single initiative prompt for the whole batch
        init_value, ok = QInputDialog.getInt(
            self, "Reinforcement Initiative",
            f"Initiative for reinforcements:\n"
            f"{summary}\n\n"
        ...
        # Convert pool entries to combatants
        added = []
        for item in self._reinforcement_pool:
            if item["type"] in ("pc", "character"):
                c = Combatant(name=item["name"])
                c.is_player = (item["type"] == "pc"
                               or bool(item.get("pc_slot_id")))
                ...
                for n in range(item["count"]):
                    ...
                    c.ac = item.get("ac", 0)
                    c.token_color = item.get("color", "#cc4444")
                    c.token_path = item.get("token_path", "")
                    self.state.combat.add_combatant(c)
                    added.append(c.name)

        # Clear the pool
        self._reinforcement_pool.clear()
        self._rebuild_reinforcement_pool_ui()
        self._update_reinforcement_button_state()

        # Refresh initiative display and overlay
        self._rebuild_initiative_ui()
        self.combat_changed.emit()
        self.status_message.emit(
            f"Reinforcements join the fight! ({total} combatants, "
            f"initiative {init_value})", 4000)
        self.save_requested.emit()
    ...
    def _quick_remove(self):
        """Remove a combatant from initiative entirely (DM correction)."""
        c = self._get_selected_or_current()
        if not c:
            return
        reply = QMessageBox.question(
            self, "Remove Combatant",
            f"Remove {c.name} from initiative order?\n\n"
            "(This is a DM correction, not a combat action.)",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            self.state.combat.remove_combatant(c.id)
            self._clear_detail_panel()
            ...
            self.combat_changed.emit()
            self.status_message.emit(
                f"Removed {c.name} from initiative order.", 3000)
            self.save_requested.emit()
